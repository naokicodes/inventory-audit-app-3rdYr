...
`server/routes/commissary.js` gained `POST /commissary/daily-audit`, mirroring `POST /daily-audit` (`dailyAudit.js`) closely: same request shape (`{ commissary_id, business_date, rows: [...] }`), same "only write fields actually provided" convention, same `{ ok, saved }` response. `commissary_id` is required for shape symmetry with the restaurant route but isn't a column on either target table — validated, never written. The two tables get deliberately different treatment, same as their restaurant-side twins: `commissary_opening_stock` is `INSERT OR IGNORE` (its `UNIQUE(commissary_meat_id)` has no date in the key — a one-time figure, first write wins, a later attempt is silently a no-op, never an upsert); `commissary_ending_actual` is a real upsert on `(commissary_meat_id, business_date)` (a physical count is per-day and corrigible — a same-day recount overwrites). `notes` (per row) and `created_by` (from a top-level `actor`, matching this file's existing yield-log/adjustments convention) are carried through on the ending-actual write; `photo_path` is untouched, as scoped. Not activity-log-scoped, same reasoning as `opening_stock`'s restaurant-side twin.

`public/commissary.html` gained an "Opening stock & physical count" section between the balance cards and the Adjustments section, reusing the same rows `GET /commissary/daily-audit` already fetches for the balance cards (no second fetch). Each meat row shows either an editable opening-stock input (when `r.beginning === null`, i.e. `MISSING_BEGINNING_STOCK`) or the established value as plain read-only text — a meat that already has an opening stock never shows an input that would silently be ignored. A physical-count input and an optional notes input are always shown; "Save counts" posts to the new route and reloads the balance cards. The section only renders real inputs when a specific commissary is selected (not "All commissaries"), since the write route needs one `commissary_id`; otherwise it shows a one-line hint.

Tests added to `server/routes/commissary.test.js` (dedicated fixtures `CM30`/`CM31`, business dates `2026-09-05`–`2026-09-07`, per the 24a-b isolation rule): 400s for each missing required field; a meat starting `MISSING_BEGINNING_STOCK`; an opening-stock write establishing a beginning; a second opening-stock write for the same meat silently ignored (single row survives, original value intact); the audit clearing to `MISSING_ACTUAL_COUNT` once a beginning exists; an ending-actual write producing a real `SHORTAGE`/variance; a same-day recount overwriting to one row, not a duplicate; blank/null fields skipped rather than written as zero; `created_by` carried through. Full suite: **16 files, 325/325 assertions, 0 failures** (up from 314 — `commissary.test.js` alone rose from 83 to 94).

Live-verified against a real booted server via `claude-in-chrome`: selected Commissary A, confirmed every real meat showed `MISSING_BEGINNING_STOCK` / `-` (matching the audit's finding exactly). Entered an opening stock of 100 for `M01 - Whole Chicken` — its balance card updated to `100.00` and the count-entry row switched from an editable input to a static "100.00" with `MISSING ACTUAL COUNT`. Entered a physical count of 95 — balance card updated to `95.00`, status `SHORTAGE`. Recounted the same date to 92 — balance card updated to `92.00`; confirmed directly against `inventory.db` that `commissary_opening_stock` and `commissary_ending_actual` each held exactly one row for that meat (no duplicate from the recount). No route exists to delete `commissary_opening_stock`/`commissary_ending_actual` rows (by design — the former is write-once, the latter is meant to be corrected by a same-day upsert, not removed), so the test rows were cleaned up with a direct SQL `DELETE` after verification, disclosed here per the live-verification rule. Server stopped and port 3000 confirmed free (`netstat` showed no `LISTENING` entry afterward, only `TIME_WAIT`).

---

## 2026-09-02 (Claude Code session) — Step 24c-i: yield-entry form UI (closes step 24)

`public/commissary.html` only. Surfaced the two multi-stage yield fields 24b-iv's write path already accepted but the page never sent: `output_commissary_meat_id` ("What this became", a `<select>` defaulting to a blank "Same meat" option = `NULL`) and `input_quantity` ("How many went in", the input's own count alongside its weighed kg) — both in the "Log a yield event" form and the yield log's inline row editor.

A new `allMeats` cache (populated by `loadMeats()`, previously discarded after building the two existing dropdowns) drives two derived behaviors off the selected input meat, via `meatById`: `populateOutputOptions` filters the output `<select>` to meats sharing the input's `commissary_id` (the server rejects a cross-commissary output, same as an allocation destination is filtered client-side rather than left to 400), and `inputQtyRequired` flips a hint span between "(required - how many were weighed)" and "(optional for a kg-tracked meat)" depending on the selected meat's `unit`, both re-run on every `Commissary meat` `change` event. Client-side validation blocks submit early when count is required and empty, mirroring the raw/backed weight check already there.

The yield table gained two columns (Input Count, Output) between Meat and Raw In/Backed Out; `renderRow` reads `output_code`/`output_name` (shows `(same)` when null) and `input_quantity` (`-` when null) straight from `GET /commissary/yield-log`, which already returned them from 24b-iv. `startEdit` now populates matching editable cells — a number input for the count, a `<select>` (via `populateOutputOptions`, filtered against the row's own `commissary_meat_id`) for the output — and `saveEdit` now actually sends both, which it never did before (safe only because 24b-iv's PATCH treats an absent key as "keep existing"). Per the PATCH convention, an emptied output `<select>` or a cleared count box sends an explicit `null`, not an omitted key, so both can be unset. `saveEdit`'s existing `alert(err.message)` on failure was left as-is — it already surfaces the server's validation text; nothing was swallowing it.

The point of this step: a yield row from before 24b-iv, on a unit-tracked meat, has `input_quantity` NULL, and 24b-iv correctly re-validates the *resulting* row on every PATCH — so such a row rejects **any** edit, including a notes-only one, with "input_quantity is required...". That rejection is deliberately kept (the row is actively debiting weighed kg out of a count-based balance). The row editor showing the empty count box is what makes it recoverable: the operator sees why, fills it in, and the save succeeds.

Full suite unchanged since no server file was touched: **16 files, 314/314, 0 failures**. Live-verified against a real booted server, via `claude-in-chrome` clicks and a few direct fetch calls for state assertions the DOM doesn't expose (network responses, balance-engine internals):
- Reproduced the legacy-row case exactly as scoped: inserted a yield row with `input_quantity = NULL` on `M02 - Whole Chicken Raw` (unit) directly via SQL (the only way to produce it post-24b-iv, since the write path now blocks creating a new one) — first attempt landed `business_date = ''` from a shell-quoting mistake (`VAR=x node -e "...$VAR..."` doesn't expand `$VAR` in the outer command), caught immediately via the read-back and fixed with a follow-up `UPDATE`. Opened the row in the editor, changed only the notes, saved — rejected with the server's real message reaching the operator via `alert`. Re-opened, filled in the count (40), saved — succeeded.
- Output-meat edit: set the same row's output to `M03 - Belly Slab`, saved, confirmed via `GET /commissary/yield-log` (`output_code: "M03"`). Cleared it back to "Same meat", saved, confirmed `output_commissary_meat_id: null` again (the explicit-null-not-omitted path).
- Same-meat kg event via the actual form and a real button click: `M03 - Belly Slab`, raw 10 / backed 9, output and count left alone — behavior unchanged, row shows `-` / `(same)`.
- Unit-in/kg-out event via the actual form: `M02 - Whole Chicken Raw`, count 40, raw-in 32.5 kg, backed-out 28 kg, output `M01 - Whole Chicken`. Confirmed via `GET /commissary/daily-audit`: the input meat's `usage` read `80` (both this event's 40-count debit and the legacy row's own 40-count debit, not the weighed kg), the input's `backedUp` read `30` (only the legacy row's own same-meat credit — this event's 28 kg went to the output instead), and the output meat's `backedUp` read `28` with `usage: 0` — the debit/credit split lands on the right rows.
- Cleaned up afterward: soft-deleted the two rows created through the UI via `DELETE /commissary/yield-log/:id` (present in `GET`'s output but excluded once `deleted_at` is set — same soft-delete convention as everything else on this table), hard-deleted the direct-SQL legacy row via a follow-up script (it never had an activity-log trail to begin with, having bypassed the route entirely). Server stopped, port 3000 confirmed free (`Get-NetTCPConnection -LocalPort 3000 -State Listen` returned nothing).

Only one live commissary exists in the current DB, so the cross-commissary output-filtering behavior (`populateOutputOptions` scoping to the input's own `commissary_id`) was exercised structurally (all options share one `commissary_id`, same as the mirrored allocation-destination pattern) rather than against two real commissaries with a live mismatch — the rejection path itself is already covered exhaustively server-side by `commissary.test.js`, same split 24b-iv used for its own cross-commissary case.

Closes step 24.

---

## 2026-09-02 (Claude Code session) — Step 24b-iv: yield-log write path

`output_commissary_meat_id` (24a) and `input_quantity` (24b-i) existed in the schema and were fully read by `commissaryAuditEngine.js`, but nothing wrote either column — `POST`/`PATCH /commissary/yield-log` in `server/routes/commissary.js` still only handled the original four fields. This closed that gap. `commissaryYieldEngine.js` (untouched, per the task) still divides by `raw_weight_in` for loss%, and `commissaryAuditEngine.js` (also untouched) already had the correct `COALESCE` reads from 24a/24b-i.

Both new fields are optional on `POST`, defaulting to `NULL` (identical to prior behavior). Validation, shared between `POST` and `PATCH` via `validateYieldOutputAndInputQty`: an `output_commissary_meat_id`, if given, must be an active commissary meat in the **same `commissary_id`** as the input meat (a yield event is a conversion, not a movement — no `meat_type_id`/unit match, unlike an ALLOCATION's destination rule, since the yield log is the one place a unit legitimately changes); an output equal to the input is accepted and behaves like `NULL` via the engine's own `COALESCE`. `input_quantity` is required when the source meat's `unit` is `'unit'` (every counted input is also weighed at intake, so both numbers always exist — a `NULL` here would make the engine debit weighed kg from a count-based balance, the exact bug 24b-i fixed from the other side) and stays optional for a `kg` source; when given it must be positive.

`PATCH` needed its own convention rather than reusing the existing "`undefined`/`null`/`''` all mean keep" coalesce: `NULL` is a *meaningful* value for both new columns (output = same meat; input qty = same as `raw_weight_in`), so an absent key now keeps the current value while an explicit `null` (or `''`) clears it — re-validated against the *resulting* row, not just what was sent, so clearing `input_quantity` to `null` on a unit-tracked source is rejected exactly like omitting it at creation.

`GET /commissary/yield-log` now also returns `output_commissary_meat_id`, `input_quantity`, `output_code`, and `output_name` (`null` when unset, not a fabricated echo of the source meat — same convention `GET /commissary/adjustments` already uses for its `destination_code`/`destination_name`). `computeYieldRow` (`commissaryYieldEngine.js`) only selects the columns its own math needs, so the route pulls the two new raw columns itself via the existing `getYieldLogRow` (`SELECT *`) rather than touching the engine.

Tests added to `server/routes/commissary.test.js` (dedicated fixtures, ids 20-24 and business dates 2026-09-10 onward, per the 24a-b isolation rule): same-meat default, cross-meat output, cross-commissary output rejected, inactive output rejected, output-equals-input accepted, unit-tracked source missing `input_quantity` rejected, kg source missing `input_quantity` accepted, non-positive `input_quantity` rejected, the real 40-birds/32.5kg-in/30kg-out case with `getCommissaryUsage`/`getCommissaryBackedUp` confirming the count is debited from the input and the weighed kg is credited to the output (not the input), PATCH absent-key-keeps, PATCH explicit-null-clears (both fields, including that clearing one doesn't touch the other), and PATCH-clears-input_quantity-on-unit-source rejected. Full suite: **16 files, 314/314 assertions, 0 failures** (up from 298; `commissary.test.js` alone rose from 47 to 83).

Also live-verified against a real booted server, since the last two sessions each found a real bug this way that no test caught: created three throwaway commissary meats (`ZT20` unit, `ZT21`/`ZT22` kg) under the one live commissary via `POST /api/settings/commissary-meats`, then exercised — over real HTTP, not direct SQL — the unit-source-without-`input_quantity` rejection, a valid 40-unit/32.5kg-in/30kg-out create, a plain kg same-meat create, `GET /commissary/yield-log`'s new `output_code`/`output_name` fields, `GET /commissary/daily-audit` confirming `usage: 40` on the input meat and `backedUp: 30` on the output meat, `PATCH` absent-key-keeps, `PATCH` explicit-null-clears (output cleared without disturbing `input_quantity`), `PATCH` rejecting a null `input_quantity` on the unit source, and the inactive-output rejection (deactivated `ZT21` via `PUT`, confirmed the create was then rejected). Afterwards soft-deleted both throwaway yield-log rows via `DELETE`, deactivated all three throwaway commissary meats via `PUT` (no delete route exists for `commissary_meats`, same treatment the existing retired test meat types already get), and stopped the server, confirming port 3000 free. Did **not** create a second live commissary to live-test the cross-commissary rejection specifically — that path is already covered exhaustively in the mirrored test suite, and the mirrored/live split is a verification-method choice, not a scope gap.
...
Allocate opens a form scoped to that card's meat, fetches `GET /commissary/adjustments/destinations?commissary_meat_id=`, and — since no live commissary meat has `meat_type_id` tagged yet — correctly renders "This meat has no meat type set yet, so it has no valid allocation destinations. Meat types are set in Settings." with Save disabled, confirmed live. Write-off (LOSS) has no destination field and stays fully usable regardless of tagging. Edit (kind fixed to the row's existing kind, not switchable) and Delete on the adjustments list reuse the yield-log's inline-edit and confirm-dialog conventions. Every create/edit/delete reloads both the balance cards and the adjustments list, since an allocation moves two meats' balances at once.

Live-verified end to end against a real booted server: created a LOSS (Belly Slab, 2.5kg, note), edited its quantity to 3.00, deleted it, confirmed the list and balance cards reloaded correctly each time. Temporarily tagged two real commissary meats (Belly Slab / Belly Slab Raw, both `kg`) with a throwaway meat type to exercise a real ALLOCATION end to end — confirmed via `GET /commissary/daily-audit` that the 10kg allocation correctly debited the source's `usage` and credited the destination's `backedUp`, both otherwise-null balances moving in lockstep — then deleted the adjustment and reverted both meats' `meat_type_id` back to `null` and deactivated the throwaway meat type, restoring the untagged live-data state exactly as found. Also click-tested the yield log's pre-existing Edit/Delete flow while in there (create → edit → delete, verified against `GET /commissary/yield-log` after each step) — this closes the "Commissary's own Edit/Delete UI flows are in the same boat — never fully click-tested" open item from `session-status.md`, for both the yield log and the new adjustments list.

One tooling note for whoever automates this page next: the delete confirm dialogs (both yield-log's pre-existing one and the new adjustments one) reliably froze the CDP screenshot/click channel used for this session's browser automation until the tab was force-navigated or `window.confirm` was stubbed — the underlying DELETE request still went through in both cases (confirmed via direct API reads), so this is a browser-automation-tool quirk, not an app bug.

Full 16-file suite: 298/298 assertions, 0 failures, unchanged (no server-side file touched).
...
**Fix**: changed the JOIN to `LEFT JOIN commissaries`, same choice 23c-ii-c made on `GET /api/commissary/meats`. The only place `commissary_code`/`commissary_name` get consumed for display in this route is the grouped `by_commissary` mapping (confirmed via grep — the untagged-meat rows show the meat's own `code`/`name`, unrelated to commissary identity, so nothing to change there); that mapping now falls back to `(unknown commissary #N)` using the row's `commissary_id` whenever `commissary_code`/`commissary_name` come back null, mirroring 23b-vi-b's wording and shape exactly. Also needed for correctness independent of display: `by_commissary`'s `.sort()` compares `.code` with `.localeCompare()`, which would throw on a literal `null` — the fallback string fixes that too.

**Verified**: baseline full suite green first (15 files, 275/275 assertions, run individually via `node <file>.test.js` — includes `server/db/activityLog.test.js` and `server/db/migrate.test.js`, easy to miss since they live outside `server/routes`/`server/engines`). Added a test to `server/routes/dashboard.test.js` (a fixture `commissary_meat` with `commissary_id: 9999`, FKs toggled off for the one insert, same pattern as the file's existing Ghost Cut/dangling-`meat_type_id` fixture) and confirmed it **failed** against the pre-fix INNER JOIN (the row was missing from `by_commissary` entirely) before applying the fix. Applied the identical LEFT JOIN + fallback change to the test file's own mirrored `stockRollup()` function, since this test file re-implements the route's logic rather than calling it over HTTP, same as this project's other route tests. Re-ran full suite after: 15 files, 276/276 assertions, 0 failures. Live-checked the booted server's real `GET /api/dashboard/stock-rollup` against the actual `inventory.db` (which always runs with `PRAGMA foreign_keys=ON`, so the dangling case itself isn't reproducible against it) to confirm no regression on normal data; server process stopped before ending the session (rule 21).

---

## 2026-09-01 (Claude Code session) — Fix: qualified-branch silent-first-match in resolveCommissaryMeat (follow-on to 23c-ii-d)

Contained correctness fix, flagged as an open item in `session-status.md` right after 23c-ii-d landed. `resolveCommissaryMeat`'s bare-token branch correctly returns `resolved` only on exactly one match and `ambiguous` on more, but the qualified `<commissary>/<meat>` branch instead did `matches.length >= 1 ? resolved(matches[0]) : unknown` — the exact silent-first-match pattern 23c-ii-d was written to eliminate, reintroduced inside its own fix. Reachable because `commissary_meats` is `UNIQUE (commissary_id, code)` with no constraint on `name`, and the qualified branch matches the meat half by code OR name — so two meats under the same commissary sharing a name (different codes) make a qualified token like `com-a/jowl` ambiguous, and the old code picked the first one silently. The submit path (`terminal.html` ~L761) and `validateCommitted` both trust this resolver, so an ambiguous qualified token would have silently shipped against the wrong `commissary_meat_id`.

**Fix**: `public/terminal.html`'s qualified branch now mirrors the bare branch's shape exactly (`=== 1` → resolved, `> 1` → ambiguous, else unknown) — nothing else in the function changed. `server/routes/terminal.test.js`'s mirrored resolver updated identically, since its own header promises it matches terminal.html "exactly." Also reworded `ambiguousCommissaryMeatMessage` (was `"${token}" matches N commissaries - qualify it, e.g. ...`, wrong wording for a same-commissary collision that has nothing to do with multiple commissaries) to `"${token}" is ambiguous - matches N meats. Qualify by code, e.g. ...`. No test asserts the exact string.

**Verified**: baseline full suite green first (15/15 files). Added a fixture the existing tests never exercised — a second COM-A meat (`comAJowl2`, code `M06`) sharing `comAJowl`'s name `JOWL` — and two new tests: `com-a/jowl` returns `ambiguous` with both matching ids, and both code-qualified forms (`com-a/m05`, `com-a/m06`) still resolve uniquely. Confirmed both new tests **failed** against the pre-fix resolver (one came back `resolved` instead of `ambiguous`) before applying the fix, then confirmed all 15/15 files green after. The existing bare-ambiguous "jowl" test's expected match count/ids were updated from 2→3 ids `[1,2]`→`[1,2,5]` since the new fixture also matches the bare branch (by name, across all three commissaries), which is correct existing behavior, not a regression. The existing "qualified token matches by name" test was repointed to a non-colliding meat (`com-a/pata`) since its original token (`com-a/jowl`) is now legitimately ambiguous. No server needed — pure resolver-logic fix, no DB/route involved. Committed as two commits: the resolver-shape fix (both files), then the message wording separately.

---

## 2026-09-01 (Claude Code session) — Step 23c-ii-d: Terminal qualified-token commissary-meat grammar

`public/terminal.html` only, the last of the four 23c-ii sub-steps and the only one closing a correctness bug rather than adding a control. `schema.sql`'s `commissary_meats.code` is `UNIQUE (commissary_id, code)`, not globally unique, so the same code (e.g. `M05`) can legitimately exist under two commissaries once a second one is created (23c-i's Commissary-creation UI). `resolveExact`'s `list.find(...)` silently returned whichever row came back first, with no warning anywhere on screen — unreachable while COM-A was the only commissary, one admin action away from firing once a second one existed.

Added `resolveCommissaryMeat(token)`, a slot-1-specific resolver returning one of three outcomes — `resolved` (exactly one match), `ambiguous` (more than one), or `unknown` — instead of `resolveExact`'s single find-or-null. Accepts both a bare token and a qualified `<commissary-code>/<meat>` form (e.g. `com-a/m05`, `com-a/jowl`); the meat half matches by code or normalized name, same convention as `resolveExact`. Routed through all four existing `resolveExact`-against-`commissaryMeats` call sites (`validateCommitted` ~L512, `computeSlotStatus` ~L570, `tryAssemblePayload`'s submit path ~L681, the shipment-preset pair-key effect ~L866) — missing even one would let the hint bar, slot guide, and submit check disagree, which is exactly the never-disagree property the file's own design comment states as intent.

An ambiguous bare token is now an error, reusing the slot guide's existing `error` display state (red, `"M05"?`) rather than inventing a new one; the hint bar's message states how many commissaries matched and offers a qualified example. A qualified token is always accepted, even when the bare form would already have been unambiguous — never tells someone they didn't need to qualify. New `commissaryMeatToken(meat)` helper picks the token to show/insert for a given meat: the bare normalized name when that name resolves uniquely back to that exact meat, the qualified form otherwise — used both for the dropdown's `token` field (so a selection is always re-parseable) and to build the ambiguity error's example. The slot-1 dropdown now shows the qualified form, plus the commissary code in its sub-label, only for candidates that are actually ambiguous; non-colliding meats render exactly as before. A commissary meat with a null `commissary_code` (a dangling `commissary_id` — reachable, and 23c-ii-c's `LEFT JOIN` preserves such rows deliberately) has no qualified form and falls back to its bare token rather than crashing or rendering `undefined/...`. Slot 2 (restaurant) is untouched — restaurant codes are globally unique, nothing to qualify.

**Not decided, flagged per the step's own instruction rather than resolved unilaterally**: what happens if two *commissaries* ever share a code (their qualifier prefix would then itself be ambiguous). `commissaries.code` is `UNIQUE` (`schema.sql` L228), so this cannot happen today — same open question the step's spec already named without answering.

**Verified**: baseline full suite run first (14/14 files, 260/260 assertions, 0 failures — the 23c-ii-c baseline), and again after (15/15 files, 273/273 assertions, 0 failures, the 13 new tests all in the new file). New `server/routes/terminal.test.js` mirrors ONLY the resolver (`resolveCommissaryMeat`/`commissaryMeatToken`/`normalize`), no DOM, same style as `commands.test.js` — plain JS fixture objects shaped like `GET /api/commissary/meats`'s real response, no DB needed since the resolver operates on that array directly. Covers: a bare unique token resolves; a bare ambiguous token (by name and separately by code) returns `ambiguous` rather than a silent first match — the actual bug fix, unprotected without this test; a qualified token resolves the correct one of two colliding meats, matching by code or by name; a qualified token is still accepted when the bare form was already unique; an unknown token, and an unknown qualified commissary code, both come back `unknown` rather than `ambiguous`; a null-`commissary_code` meat doesn't throw and still resolves bare; `commissaryMeatToken` picks bare vs. qualified correctly in all three cases including the null-code fallback. `node --check` on the extracted inline script. Live end-to-end check against a real booted server: created a real second commissary and a commissary meat reusing the existing `M05` code, replayed the exact live resolver logic against the real `GET /api/commissary/meats` response — confirmed the bare token `m05` now comes back `ambiguous` (refused, not silently resolved), both qualified forms (`com-a/m05`, `com-test/m05`) resolve to the correct distinct meat, and a non-colliding meat (`M01`) still resolves bare exactly as before. Test rows cleaned up via direct DB delete afterward (no DELETE route for either table, expected). Server process stopped before ending the session (rule 21). Pushed directly to `main`.
